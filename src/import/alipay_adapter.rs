...
impl BillImportAdapter for AlipayAdapter {
    fn format_name(&self) -> &'static str {
        "支付宝"
    }

    fn detect(&self, header: &str) -> bool {
        header.contains("交易时间") && header.contains("商品说明")
    }
    ...
    fn parse(&self, content: &str) -> AppResult<ImportResult> {
        ...
        let desc_col = CsvParser::find_column(&headers, &["商品说明"]);
        ...
        for row in rows.iter() {
            ...
            // 解析金额
            let amount = amount_str.trim().replace(',', "");
